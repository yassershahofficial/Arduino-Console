...
//Total bytes used to store images in PROGMEM = 192
...
/* ---------------------All Mov (Player, Bot, Ball)--------------------------------------- */
void playerMov(void){
  if(playerY >= limitPlayerYmax){
    playerYdir = -1;
    playerTrajectory();
    changeOnPlayer = 3;
  }
  ...
  else if(changeOnPlayer == 0){
    playerYdir = 1;
    playerTrajectory();
  }
  else if(changeOnPlayer == 1){
    playerYdir = -1;
    playerTrajectory();
  }
  else{
...
/* --------------User Input (Key --> UP, DOWN, ACT A, ACT B)------------------------------ */
void key(){
...
/* ----------------All Visual (Scoreboard, Static Draw, Draw)----------------------------- */
